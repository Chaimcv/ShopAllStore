...
console.log(data,"data");
  ...
  return (
    ...
    <div className="bg-white  hidden lg:grid flex grid grid-cols-4 mx-[3%] my-[3%]"> 
       ...
       <p>₹{items?.price}</p>
       <Link to={`/details/${items.id}`}><button className='bg-black text-white m-1 rounded-2xl'>View Details</button></Link>
       <button className='bg-black text-white m-1 rounded-2xl' onClick={()=>Addtocart(items)}>Add to cart</button>
      ...
      {data?.map((items)=>(
       ...
       <img className="image w-[60%] h-[60%] mx-auto "src={items?.image}></img>
       <p>₹{items?.price}</p>
       <Link to={`/details/${items.id}`}><button className='bg-black text-white m-1 rounded-2xl' >View Details</button></Link><br />
        <button className='bg-black text-white m-1 rounded-2xl' onClick={()=>Addtocart(items)}>Add to cart</button>
